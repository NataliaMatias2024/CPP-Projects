...
/*   Updated: 2026/09/18 13:39:19 by namatias         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ScalarConverter::convert(const std::string &literal)
{
	...
	*/
	const std::string pseudoLiterals[6] = {"-inff", "+inff", "nanf",
		"-inf", "+inf", "nan"};
	for (int i = 0; i < 6; i++)
	{
	...
	*/
	...
	else
	{
		...
		*/
		char *end; // Vai armazenar o ultimo caracter APOS conversao em double
		errno = 0;
		double auxD = std::strtod(literal.c_str(), &end);
		if (errno == ERANGE || (*end != 'f' && *end != '\0')
			|| (*end == 'f' && *(end + 1) != '\0') || end == literal.c_str())
		{
